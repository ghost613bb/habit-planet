...
function createLeafClump() {
  ...
  const leafGeo = new SphereGeometry(0.17, 7, 6)

  const leaf1 = new Mesh(leafGeo, leafMat)
  leaf1.position.set(0.1, 0.12, 0.02)
  leaf1.scale.set(1.15, 0.82, 1.02)
  leaf1.userData.flowerBushRole = 'leaf'

  const leaf2 = new Mesh(leafGeo, leafMat)
  leaf2.position.set(-0.1, 0.1, -0.08)
  leaf2.scale.set(1.03, 0.8, 0.95)
  leaf2.userData.flowerBushRole = 'leaf'

  const leaf3 = new Mesh(leafGeo, leafMat)
  leaf3.position.set(0.02, 0.09, 0.11)
  leaf3.scale.set(1, 0.78, 0.98)
...
function createPetalCross(palette: Palette) {
  ...
  const plane = new PlaneGeometry(0.22, 0.11, 1, 1)
  ...
  const center = new Mesh(new SphereGeometry(0.05, 6, 6), centerMat)
...
function createProceduralTemplate() {
  ...
  const anchors = [
    // 1 朵居中偏上，4 朵环绕四周，保证主视角下也能清晰看出 5 朵花。
    { x: 0.0, y: 0.34, z: 0.0, yaw: 0.0, scale: 1.1 },
    { x: 0.17, y: 0.27, z: 0.04, yaw: 0.24, scale: 1.02 },
    { x: -0.16, y: 0.27, z: -0.03, yaw: 0.92, scale: 1.02 },
    { x: -0.02, y: 0.27, z: 0.18, yaw: -0.52, scale: 0.98 },
    { x: 0.03, y: 0.25, z: -0.18, yaw: 0.48, scale: 0.98 },
  ] as const
...
export function createFlowerBushInstance(options?: {
  ...
  const targetHeight = options?.targetHeight ?? 0.92
