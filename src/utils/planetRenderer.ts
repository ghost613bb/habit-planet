...
export function createPlanetRenderer(input: {
  ...
  const planetRadius = 3.0
  const grassRadius = 3.05

  refs.planetMesh = new Mesh(new SphereGeometry(planetRadius, 64, 64), mats.planet)
  refs.planetMesh.receiveShadow = true
  planetGroup.add(refs.planetMesh)

  refs.grassMesh = new Mesh(new SphereGeometry(grassRadius, 64, 64), mats.grass)
